...
async function refreshJournalEntriesFromServer() {
  try {
    ...
    console.error('Nie udało się zsynchronizować wpisów społeczności:', error);
  }
}
...
function connectJournalRealtimeUpdates() {
  ...
  try {
    ...
    console.error('Nie udało się nawiązać połączenia z kanałem społeczności:', error);
  ...
  journalEventSource.addEventListener('journal-entry-created', (event) => {
    try {
      ...
      console.error('Nie udało się przetworzyć aktualizacji dziennika społeczności:', error);
    }
  });
...
function createJournalEntryElement(entry, options = {}) {
  ...
  if (isOwnEntry) {
    ...
    if (!currentUserKey) {
      ...
      likeBtn.title = 'Zaloguj się, aby polubić wpis społeczności.';
    } else {
  ...
  if (shouldRenderComments) {
    ...
    titleEl.textContent = 'Komentarze społeczności';
    ...
    if (!currentUserKey) {
      ...
      loginText.textContent = 'Zaloguj się, aby dodać komentarz i polubić wpis społeczności.';
...
function startJournalCommentEdit(entryId, commentId) {
  ...
  if (!currentUserKey) {
    setLevelStatus('Zaloguj się, aby edytować komentarze społeczności.', 5000);
    openAuthModal();
    return;
  }
...
function handleJournalCommentDelete(entryId, commentId) {
  ...
  if (!currentUserKey) {
    setLevelStatus('Zaloguj się, aby usuwać komentarze społeczności.', 5000);
    openAuthModal();
    return;
  }
...
function toggleJournalEntryLike(entryId) {
  ...
  if (!currentUserKey) {
    setLevelStatus('Zaloguj się, aby polubić wpisy społeczności.', 5000);
    return;
  }
...
function handleJournalCommentSubmit(event, entryId, parentCommentId = null) {
  ...
  if (!currentUserKey) {
    setLevelStatus('Zaloguj się, aby dodać komentarz w strefie społeczności.', 5000);
    openAuthModal();
    return;
  }
...
function renderJournalEntries() {
  ...
  const targets = [
    ...
    { listId: 'communityJournalList', emptyId: 'communityJournalEmptyMessage' },
    {
...
async function handleJournalFormSubmit(event) {
  ...
  if (!currentUserKey) {
    setJournalFormMessage('Zaloguj się, aby dodać wpis w strefie społeczności.', 'error');
    openAuthModal();
    return;
  }
  ...
  setJournalFormMessage('Publikuję Twój wpis w społeczności…', 'info');

  let savedEntry;
  try {
    savedEntry = await createCommunityJournalEntry(payload);
  } catch (error) {
    console.error('Nie udało się opublikować wpisu społeczności:', error);
    const errorMessage =
