-- =====================================================
-- ADMIN CONTENT MANAGEMENT - COMPLETE INSTALLATION
-- =====================================================
-- Ten plik zawiera WSZYSTKIE wymagane zależności
...
-- =====================================================
-- PART 3: CONTENT MANAGEMENT FUNCTIONS
-- =====================================================
...
-- Function: Get detailed content statistics
...
-- Check if is_current_user_admin exists
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_proc WHERE proname = 'is_current_user_admin'
  ) THEN
    RAISE NOTICE '✅ Function is_current_user_admin() created';
  ELSE
    RAISE EXCEPTION '❌ Function is_current_user_admin() NOT created';
  END IF;
END $$;
...
BEGIN
  IF EXISTS (
    ...
    RAISE NOTICE '✅ Table admin_actions created';
  ELSE
    RAISE EXCEPTION '❌ Table admin_actions NOT created';
  END IF;
END $$;

-- Count content management functions
DO $$
DECLARE
  func_count INTEGER;
BEGIN
  ...
  IF func_count = 8 THEN
    RAISE NOTICE '✅ All 8 content management functions created';
  ELSE
    RAISE WARNING '⚠️  Only % of 8 content management functions created', func_count;
  END IF;
END $$;
...
BEGIN
  ...
  RAISE NOTICE '  - 8 content management functions';
  RAISE NOTICE '';
  RAISE NOTICE 'Next steps:';
  RAISE NOTICE '  1. Reload admin panel: https://cypruseye.com/admin';
  RAISE NOTICE '  2. Go to Content tab';
  RAISE NOTICE '  3. All features should work now!';
  RAISE NOTICE '=====================================================';
END $$;
