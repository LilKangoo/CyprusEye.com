...
## Car booking (szkielety)

Pliki:
- `public/auto-larnaka.html`, `public/auto-paphos.html`, `public/admin.html`, `public/assets/app.js`
- `supabase/migrations/000_init_car_booking.sql`
- Edge Functions: `supabase/functions/send_offer_email/`, `supabase/functions/stripe_webhook/`

Env (Supabase):
- `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`
- `SMTP_*` (Zoho), `SITE_URL`
- `STRIPE_SECRET_KEY`, `DEPOSIT_PER_DAY=5`
...
Instrukcje skrócone:
1) Uruchom migrację: zawartość `supabase/migrations/000_init_car_booking.sql` (tabele, RLS, helper `is_admin()`).
2) Wdróż Edge Functions (TODO: implementacja kodu): `send_offer_email`, `stripe_webhook` i ustaw webhook Stripe na funkcję `stripe_webhook`.
3) Serwuj strony publiczne (Larnaka/Pafos) i admin (`public/admin.html`). Admin wymaga logowania oraz `user_metadata.is_admin=true` (TODO: egzekwowanie w kodzie).
4) Formularze łączą się z tabelami `cars` i `bookings` (TODO: logika w `public/assets/app.js`).

Status: to szkielety – sekcje oznaczone `TODO: implementacja` wymagają uzupełnienia logiką.
