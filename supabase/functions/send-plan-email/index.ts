...
function t(lang: SupportedLang, key: string): string {
  const dict: Record<SupportedLang, Record<string, string>> = {
    en: {
      subjectPrefix: "Your Trip Plan",
      preheader: "Your Cyprus trip plan is ready.",
      base: "Base",
      dates: "Dates",
      email: "Email",
      day: "Day",
      noItems: "No items.",
      generatedBy: "Generated by CyprusEye Trip Planner.",
      openCyprusEye: "Open CyprusEye",
    },
    pl: {
      subjectPrefix: "Plan podróży",
      preheader: "Twój plan podróży na Cypr jest gotowy.",
      base: "Baza",
      dates: "Daty",
      email: "E-mail",
      day: "Dzień",
      noItems: "Brak pozycji.",
      generatedBy: "Wygenerowano w planerze podróży CyprusEye.",
      openCyprusEye: "Otwórz CyprusEye",
    },
  };
...
function buildPlanEmailHtml(params: {
  ...
  const { recipientEmail, plan, days, itemsByDayId, lang } = params;
  ...
  const subject = `${t(lang, "subjectPrefix")} – ${title}`;

  const preheader = [
    t(lang, "preheader"),
    baseCity ? `${t(lang, "base")}: ${baseCity}.` : "",
    dateRange ? `${t(lang, "dates")}: ${dateRange}.` : "",
  ]
    .filter(Boolean)
    .join(" ");
  ...
  const headerMetaParts = [
    baseCity ? `${t(lang, "base")}: ${baseCity}` : "",
    dateRange ? `${t(lang, "dates")}: ${dateRange}` : "",
    recipientEmail ? `${t(lang, "email")}: ${recipientEmail}` : "",
  ].filter(Boolean);
  ...
  const daySections = (days || []).map((d) => {
    ...
    const dayTitleParts = [
      `${escapeHtml(t(lang, "day"))} ${escapeHtml(String(d?.day_index || ""))}`,
    ...
    const itemsHtml = items.length
      ? items
            ...
            return `
              <tr>
                <td style="padding:10px 0; border-top:1px solid #e5e7eb;">
                  ...
                  ${description ? `<div style="margin-top:6px; font-size:12px; color:#475569;">${escapeHtml(description)}</div>` : ""}
          ...
          .join("")
      : `<tr><td style="padding:10px 0; color:#64748b; font-size:12px;">${escapeHtml(t(lang, "noItems"))}</td></tr>`;
              ...
              <tr>
                <td style="padding:14px 20px 18px 20px; border-top:1px solid #e5e7eb;">
                  ...
                  <div style="margin-top:8px; font-family:system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial; font-size:12px; color:#64748b; line-height:1.5;">
                    ${escapeHtml(t(lang, "openCyprusEye"))}: <a href="${escapeHtml(CUSTOMER_HOMEPAGE_URL)}" style="color:#2563eb; text-decoration:underline;">${escapeHtml(CUSTOMER_HOMEPAGE_URL)}</a>
                  </div>
                </td>
              </tr>
            </table>
  ...
  const textLines = [
    ...
    ...(days || []).flatMap((d) => {
      ...
      const items = (itemsByDayId.get(dayId) || [])
        .slice()
        .sort((a, b) => Number(a?.sort_order || 0) - Number(b?.sort_order || 0));
      const dayHeader = `${t(lang, "day")} ${d?.day_index || ""} ${d?.date || ""} ${d?.city || ""}`.trim();
      const itemLines = items.map((it) => {
        const data = it?.data && typeof it.data === "object" ? it.data : {};
        const itemTitle = String(data?.title || data?.name || it?.item_type || "Item");
        const description = String(data?.description || "").trim();
        const desc = description ? ` — ${description}` : "";
        return `- ${itemTitle}${desc}`;
      });
      return [dayHeader, ...(itemLines.length ? itemLines : [`- ${t(lang, "noItems")}`]), ""]; 
    }),
  ].filter(Boolean);
...
serve(async (req) => {
  ...
  try {
    ...
    const email = buildPlanEmailHtml({
      recipientEmail,
      plan,
      days,
      itemsByDayId,
