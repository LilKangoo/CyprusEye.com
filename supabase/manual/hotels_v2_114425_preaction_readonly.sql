...
-- Final successor 114425 preaction; accepted base c081653a7806651f5d5f06c704420bb976652030.
...
-- Expected rows: 145 (144 required leaves and one summary).
...
specs(ordinal,section,leaf_name,expected,required_relations,required_functions,read_query) AS (
 VALUES
  ...
  (122,'114425_collision','function_name_cardinality_exact','true',
    ARRAY[]::text[],ARRAY[]::text[],
    $check122$SELECT ((SELECT count(*) FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='public' AND p.proname='hotel_v2_external_calendar_site_settings_fingerprint')=0)::text AS actual$check122$),
...
leaves AS MATERIALIZED (
 ...
 FROM measurements
 UNION ALL
 SELECT 130+row_number() OVER(ORDER BY signature)::integer,'source_security',signature,'true',exact::text,exact IS TRUE,safe_context FROM pin_results
),
totals AS (
...
SELECT section,ordinal,leaf_name,expected,actual,pass,safe_context,
 NULL::boolean AS "PREACTION_READY",NULL::jsonb AS blocker_codes,NULL::bigint AS required_leaf_count,NULL::bigint AS passed_leaf_count
FROM leaves
UNION ALL
SELECT 'final_gate',145,'PREACTION_READY','true',ready::text,ready,
 '114425_PREACTION_ONLY; NOT_WRITE_AUTHORIZATION; SQL_BACKUP_PROOF=NO',ready,blockers,required_count,passed_count FROM totals
ORDER BY ordinal;
ROLLBACK;
