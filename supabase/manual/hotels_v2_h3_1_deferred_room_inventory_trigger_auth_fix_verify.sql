...
), all_h3_constraint_triggers as (
  select count(*) filter(where trigger_row.tgenabled='O'
      and trigger_row.tgdeferrable and trigger_row.tginitdeferred) enabled_deferred_count,
    count(*) total_count
  from pg_trigger trigger_row
  where not trigger_row.tgisinternal and trigger_row.tgname in (
    'hotel_room_allocation_rules_contract_guard',
    'hotel_room_allocation_rule_items_contract_guard',
    'hotel_room_types_h3_1_allocation_inventory_guard',
    'hotel_units_h3_1_allocation_inventory_guard',
    'hotel_payment_policies_contract_guard',
    'hotel_payment_policy_terms_contract_guard'
  )
), rpc_state as (
...
select
  ...
  all_h3_constraint_triggers.enabled_deferred_count,
  all_h3_constraint_triggers.total_count,
  rpc_state.expected_rpc_count,
  capability_state.flags_off,
  coalesce((
    function_state.entry_owner='postgres'
    and function_state.entry_kind='f'
    and function_state.entry_security_definer
    ...
    and not has_function_privilege('public',function_state.entry_oid,'EXECUTE')
    and not has_function_privilege('anon',function_state.entry_oid,'EXECUTE')
    and not has_function_privilege('authenticated',function_state.entry_oid,'EXECUTE')
    and not has_function_privilege('service_role',function_state.entry_oid,'EXECUTE')
    and not has_function_privilege('authenticator',function_state.entry_oid,'EXECUTE')
    ...
    and all_h3_constraint_triggers.enabled_deferred_count=6
    and all_h3_constraint_triggers.total_count=6
    and rpc_state.expected_rpc_count=3
    and capability_state.flags_off
  ),false) hotels_v2_h3_1_deferred_room_inventory_trigger_auth_safe
from function_state cross join trigger_state cross join all_h3_constraint_triggers
cross join rpc_state cross join capability_state;
