...
room_state as (
  select count(*)::integer room_count,
    count(*) filter(where room.id=constants.upper_id and room.status='active'
      ...
      and jsonb_array_length(room.gallery)=6)::integer upper_exact_count,
    count(*) filter(where room.id=constants.ground_id and room.status='active'
      ...
      and jsonb_typeof(room.gallery)='array'
      and jsonb_array_length(room.gallery)=5)::integer ground_exact_count,
...
room_gallery_state as (
  select
    ...
    (select count(*)::integer from (
      ...
      from room_gallery_rows duplicate
      group by duplicate.room_id,duplicate.photo
      having count(*)>1
    ) repeated)::integer duplicate_photo_count
  from room_gallery_rows room_photo cross join constants
),
unit_state as (
  select count(*)::integer unit_count
...
rate_state as (
  select
    ...
    (select count(*) from public.hotel_room_rates rate cross join constants
      where rate.hotel_id=constants.hotel_id and rate.is_active)::integer active_room_rate_count,
    (select count(*) from public.hotel_room_rates rate cross join constants
      where rate.id=constants.upper_rate_id and rate.hotel_id=constants.hotel_id
        ...
        and rate.external_redirect_url is null and not rate.is_active)::integer upper_room_rate_count,
    (select count(*) from public.hotel_room_rates rate cross join constants
      where rate.id=constants.ground_rate_id and rate.hotel_id=constants.hotel_id
        ...
        and rate.pricing_schedule_id=constants.room_schedule_id
        and rate.base_nightly_rate=0 and rate.currency='EUR'
        and rate.external_redirect_url is null and not rate.is_active)::integer ground_room_rate_count,
    ...
    (select count(*) from public.hotel_pricing_schedules schedule cross join constants
      where schedule.hotel_id=constants.hotel_id and schedule.id=constants.room_schedule_id
        ...
        and schedule.source='legacy_preview' and schedule.minimum_billable_occupancy=2)::integer room_schedule_count,
    (select count(*) from public.hotel_pricing_schedules schedule cross join constants
      where schedule.hotel_id=constants.hotel_id and schedule.id=constants.party_schedule_id
        ...
        and not schedule.is_active and schedule.review_status='requires_review'
        and schedule.source='legacy_preview' and schedule.minimum_billable_occupancy=2)::integer party_schedule_count,
    ...
    (select count(*) from public.hotel_pricing_schedule_occupancy_tiers tier
      join public.hotel_pricing_schedules schedule on schedule.id=tier.schedule_id
      cross join constants where schedule.hotel_id=constants.hotel_id
        and (tier.schedule_id not in (constants.room_schedule_id,constants.party_schedule_id)
  ...
  select
    ...
    (select count(*) from public.hotel_payment_policy_terms term
      ...
      where policy.hotel_id=constants.hotel_id and policy.code='seven-kamares-request-confirmation'
        and term.sequence=1 and term.due_event='after_partner_acceptance'
        and term.amount_mode='percent_total' and term.amount_value=50
        and term.recipient='partner' and term.payment_methods=array['bank_transfer']::text[]
        and exists(
          select 1 from jsonb_each_text(term.instructions_i18n) instruction
          where btrim(instruction.value)<>''
        ))::integer acceptance_instruction_count,
    (select count(*) from public.hotel_payment_policy_terms term
      join public.hotel_payment_policies policy on policy.id=term.payment_policy_id cross join constants
      where policy.hotel_id=constants.hotel_id and policy.code='seven-kamares-request-confirmation'
...
select
  ...
  property_state.property_count as seven_kamares_property_count,partners.eligible_owner_count,
  partners.operational_assignment_count,partners.owner_is_operational_count,
  rooms.room_count,gallery.upper_gallery_count,gallery.ground_gallery_count,
  gallery.foreign_photo_count,gallery.duplicate_photo_count,units.unit_count,
  rates.total_rate_plan_count,rates.rate_plan_count,
  rates.total_room_rate_count,rates.inactive_room_rate_count,rates.active_room_rate_count,
  rates.total_schedule_count,rates.room_schedule_count,rates.party_schedule_count,
  rates.total_tier_count,rates.room_tier_count,rates.party_tier_count,
  rates.unexpected_or_inactive_tier_count,
  ...
  payments.policy_count,payments.term_count,payments.acceptance_instruction_count,
  (payments.acceptance_instruction_count=1) partner_bank_instructions_ready,
  jsonb_strip_nulls(jsonb_build_object(
    'partner_bank_payment_instructions',case when payments.acceptance_instruction_count<>1 then 'missing' end,
    'rate_plan',case when rates.rate_plan_count=1 then 'inactive' end,
    'room_rates',case when rates.active_room_rate_count<>2 then 'inactive' end,
  ...
  commercial.commission_count,commercial.manual_source_count,
  ...
  (
    ...
    and units.unit_count=0
    and rates.total_rate_plan_count=1 and rates.rate_plan_count=1
    and rates.total_room_rate_count=2 and rates.inactive_room_rate_count=2
    and rates.active_room_rate_count=0
    and rates.upper_room_rate_count=1 and rates.ground_room_rate_count=1
    and rates.total_schedule_count=2
    and rates.room_schedule_count=1 and rates.party_schedule_count=1
    and rates.total_tier_count=90
    and rates.room_tier_count=27 and rates.party_tier_count=63
    and rates.unexpected_or_inactive_tier_count=0 and rates.fingerprint_count=2
    and room_parity.mismatch_count=0 and party_parity.mismatch_count=0
    and legacy.case_count=70 and legacy.mismatch_count=0
    ...
    and commercial.enabled_external_count=0
    and flags.settings_count=1 and flags.all_off_count=1
...
from constants cross join properties cross join property_state
cross join partner_state partners cross join room_state rooms
cross join room_gallery_state gallery cross join unit_state units cross join rate_state rates
...
cross join calendar_state calendar
cross join room_schedule_mismatch room_parity
cross join party_schedule_mismatch party_parity
cross join legacy_oracle legacy cross join flags cross join history cross join oracle;
