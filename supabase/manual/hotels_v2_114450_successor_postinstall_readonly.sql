...
BEGIN
 ...
 IF (SELECT count(*) FROM hotels_lineage_private.successor_receipts)<>1
 OR (SELECT array_agg(stage ORDER BY stage) FROM hotels_lineage_private.successor_receipts)
    IS DISTINCT FROM ARRAY[114450]
 OR hotels_lineage_private.successors_are_exact(root_hash) IS NOT TRUE
 OR hotels_lineage_private.successor_boundary_is_exact(114450,true) IS NOT TRUE
 ...
 THEN RAISE EXCEPTION 'hotels_114450_successor_certificate_mismatch'; END IF;
END $successor$;
SELECT 'HOTELS_114450_SUCCESSOR_CERTIFICATE_OK' AS sentinel,
 current_setting('transaction_read_only') AS transaction_read_only,
 1 AS successor_receipt_count,true AS exact_stage_set,true AS before_after_pins_exact,
