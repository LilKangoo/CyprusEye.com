...
CREATE OR REPLACE FUNCTION pg_temp.he_beta_set_he_if_empty(target jsonb, value text)
RETURNS jsonb
LANGUAGE sql
AS $$
  SELECT CASE
...
CREATE OR REPLACE FUNCTION pg_temp.he_beta_set_he_array_if_empty(target jsonb, value text[])
RETURNS jsonb
LANGUAGE sql
AS $$
  SELECT CASE
...
CREATE OR REPLACE FUNCTION pg_temp.he_beta_doc(VARIADIC paragraphs text[])
RETURNS jsonb
LANGUAGE sql
AS $$
...
UPDATE public.pois
SET
  name_i18n = pg_temp.he_beta_set_he_if_empty(name_i18n, 'חוף פיניקודס - הלב של לרנקה'),
  description_i18n = pg_temp.he_beta_set_he_if_empty(description_i18n, 'טיילת וחוף עירוני מרכזי בלרנקה, עם דקלים, מים רגועים, מסעדות וגישה נוחה למרינה ולמבצר הימי.'),
  badge_i18n = pg_temp.he_beta_set_he_if_empty(badge_i18n, 'מטייל פיניקודס')
WHERE id = 'larnaca-beach';

UPDATE public.pois
SET
  name_i18n = pg_temp.he_beta_set_he_if_empty(name_i18n, 'מרינת לימסול - הלב האלגנטי של החוף'),
  description_i18n = pg_temp.he_beta_set_he_if_empty(description_i18n, 'מרינה מודרנית במרכז לימסול, מתאימה לטיול ערב, מסעדות, יאכטות ונוף עירוני נעים ליד הנמל הישן.'),
  badge_i18n = pg_temp.he_beta_set_he_if_empty(badge_i18n, 'מטייל במרינה')
WHERE id = 'limassol-marina';

UPDATE public.pois
SET
  name_i18n = pg_temp.he_beta_set_he_if_empty(name_i18n, 'העיר העתיקה ניקוסיה - מסע בזמן'),
  description_i18n = pg_temp.he_beta_set_he_if_empty(description_i18n, 'המרכז ההיסטורי של ניקוסיה נמצא בתוך החומות הוונציאניות ומשלב סמטאות, שערים עתיקים, בתי קפה ואת קו הגבול הירוק.'),
  badge_i18n = pg_temp.he_beta_set_he_if_empty(badge_i18n, 'בלב חומות ניקוסיה')
WHERE id = 'nicosia-old-town';

UPDATE public.pois
SET
  name_i18n = pg_temp.he_beta_set_he_if_empty(name_i18n, 'גשר האהבה'),
  description_i18n = pg_temp.he_beta_set_he_if_empty(description_i18n, 'קשת סלע טבעית באזור איה נאפה וקייפ גרקו, פופולרית לצילומים ולתצפית על מים טורקיז.'),
  badge_i18n = pg_temp.he_beta_set_he_if_empty(badge_i18n, 'גשר האהבה')
WHERE id = 'Love-Bridge';

UPDATE public.pois
SET
  name_i18n = pg_temp.he_beta_set_he_if_empty(name_i18n, 'חוף קונוס ביי'),
  description_i18n = pg_temp.he_beta_set_he_if_empty(description_i18n, 'מפרץ חולי ושקט בין איה נאפה לפרוטארס, עם מים צלולים, נוף ירוק וגישה נוחה לשחייה רגועה.'),
  badge_i18n = pg_temp.he_beta_set_he_if_empty(badge_i18n, 'מפרץ של שלווה')
WHERE id = 'konnos-bay-beach';

UPDATE public.pois
SET
  name_i18n = pg_temp.he_beta_set_he_if_empty(name_i18n, 'מערות הים של קייפ גרקו'),
  description_i18n = pg_temp.he_beta_set_he_if_empty(description_i18n, 'מערות וקליפים מרשימים בקייפ גרקו, עם נקודות צילום, מים כחולים ואפשרות לצפייה מהחוף או מהים.'),
  badge_i18n = pg_temp.he_beta_set_he_if_empty(badge_i18n, 'מחפש מערות')
WHERE id = 'cape-greco-sea-caves';

UPDATE public.pois
SET
  name_i18n = pg_temp.he_beta_set_he_if_empty(name_i18n, 'מערת הקיקלופ'),
  description_i18n = pg_temp.he_beta_set_he_if_empty(description_i18n, 'מערה קטנה ליד קייפ גרקו ופרוטארס, הקשורה במסורת המקומית לסיפור אודיסאוס והקיקלופ פוליפמוס.'),
  badge_i18n = pg_temp.he_beta_set_he_if_empty(badge_i18n, 'בריחה ממערת פוליפמוס')
WHERE id = 'Cyclops-Cave';

UPDATE public.pois
SET
  name_i18n = pg_temp.he_beta_set_he_if_empty(name_i18n, 'כנסיית סנט לזרוס בלרנקה'),
  description_i18n = pg_temp.he_beta_set_he_if_empty(description_i18n, 'כנסיית אבן מהמאה התשיעית בלב לרנקה, ידועה באיקונוסטזיס המרשים ובקשר למסורת על קברו של לזרוס.'),
  badge_i18n = pg_temp.he_beta_set_he_if_empty(badge_i18n, 'שומר השרידים')
WHERE id = 'saint-lazarus-church';

UPDATE public.pois
SET
  name_i18n = pg_temp.he_beta_set_he_if_empty(name_i18n, 'העיר העתיקה פמגוסטה'),
  description_i18n = pg_temp.he_beta_set_he_if_empty(description_i18n, 'עיר מבוצרת עם חומות ונציאניות, קתדרלה היסטורית, מבצר אותלו ואווירה חזקה של עבר ימי ביניימי.'),
  badge_i18n = pg_temp.he_beta_set_he_if_empty(badge_i18n, 'כרוניקן פמגוסטה')
WHERE id = 'famagusta-old-town';

UPDATE public.pois
SET
  name_i18n = pg_temp.he_beta_set_he_if_empty(name_i18n, 'מבצר לרנקה'),
  description_i18n = pg_temp.he_beta_set_he_if_empty(description_i18n, 'מבצר ימי קטן בקצה טיילת פיניקודס, ששימש לאורך השנים להגנה על הנמל, תחנת ארטילריה, בית סוהר ומוזיאון.'),
  badge_i18n = pg_temp.he_beta_set_he_if_empty(badge_i18n, 'שומר הנמל')
...
UPDATE public.trips
SET
  title = pg_temp.he_beta_set_he_if_empty(title, 'הלגונה הכחולה'),
  description = pg_temp.he_beta_set_he_if_empty(description, 'טיול יום פרטי מאזור פאפוס אל הלגונה הכחולה: עצירות נוף, נסיעת שטח, זמן לשחייה ושנורקלינג, מים וכיבוד קל ברכב. מתאים למטיילים שרוצים יום טבע רגוע עם נהיגה מודרכת.')
WHERE id = '47fd4793-647b-45fd-a2ce-1ecaa4b95922';

UPDATE public.trips
SET
  title = pg_temp.he_beta_set_he_if_empty(title, 'מסלול סלע אפרודיטה'),
  description = pg_temp.he_beta_set_he_if_empty(description, 'טיול יום פרטי מפאפוס דרך נקודות טבע וחוף סביב סלע אפרודיטה: תצפיות, נהיגת שטח, עצירות צילום וחוויה קפריסאית מחוץ למסלול הרגיל.')
WHERE id = '2d937b4f-3da7-4fed-bc06-bdc66eb25612';

UPDATE public.trips
SET
  title = pg_temp.he_beta_set_he_if_empty(title, 'טרודוס ולפקרה'),
  description = pg_temp.he_beta_set_he_if_empty(description, 'טיול יום אל הרי טרודוס ולפקרה, עם כפרים מסורתיים, נקודות תצפית, מפלים ואווירה הררית שונה מהחופים של קפריסין.')
...
SET
  title = pg_temp.he_beta_set_he_if_empty(title, 'RGB Cabins - מרכז לרנקה'),
  description = pg_temp.he_beta_set_he_if_empty(description, 'יחידות סטודיו במרכז לרנקה, קרובות לחוף פיניקודס, למסעדות ולטיילת. מתאים לזוגות, יחידים או קבוצות שרוצות לשהות יחד במיקום עירוני נוח.')
WHERE id = 'f9fbaa61-fdce-4418-8579-ddb2b0a75fb1';

UPDATE public.hotels
SET
  title = pg_temp.he_beta_set_he_if_empty(title, '7 Arches'),
  description = pg_temp.he_beta_set_he_if_empty(description, 'אירוח בלפקרה באווירה קפריסאית שקטה, עם מיקום נוח לטיולים בכפרים ובהרים. מתאים למי שמחפש בסיס אותנטי ונעים מחוץ לערים הגדולות.')
...
SET features = pg_temp.he_beta_set_he_array_if_empty(features, ARRAY[
  'מיזוג אוויר',
  'אוטומטי',
  '8 מקומות',
  'דלתות הזזה',
...
BEGIN
  IF EXISTS (
    ...
    EXECUTE $sql$
      UPDATE public.car_offers
      SET description_i18n = pg_temp.he_beta_set_he_if_empty(description_i18n, 'רכב 8 מקומות מרווח למשפחות או קבוצות, מתאים לנסיעות באזור פאפוס עם דלתות הזזה ונוחות גבוהה.')
      WHERE id = '353d8c79-eb1f-4c1e-9c4a-59febf2ea7ca';

      UPDATE public.car_offers
      SET description_i18n = pg_temp.he_beta_set_he_if_empty(description_i18n, 'SUV נוח עם תא נוסעים איכותי, מתאים לנסיעות ארוכות ולמשפחות שרוצות רכב גבוה ונעים.')
      WHERE id = 'b4f784d3-22d2-421a-829f-2394e3a72a76';

      UPDATE public.car_offers
      SET description_i18n = pg_temp.he_beta_set_he_if_empty(description_i18n, 'רכב 7 מקומות עם הנעה כפולה ומרווח טוב, מתאים לטיולים מחוץ לעיר ולמשפחות גדולות.')
      WHERE id = '8a1158af-6b05-4723-b2eb-93b130d22f24';

      UPDATE public.car_offers
      SET description_i18n = pg_temp.he_beta_set_he_if_empty(description_i18n, 'רכב 7 מקומות מודרני ונוח בלרנקה, עם דלתות הזזה וטעינת USB לנסיעות משפחתיות.')
      WHERE id = '5ba581c3-08c6-47cd-ab29-4d2b9213cebc';

      UPDATE public.car_offers
      SET description_i18n = pg_temp.he_beta_set_he_if_empty(description_i18n, 'רכב היברידי קומפקטי וחסכוני, מתאים לזוגות או משפחות קטנות שמטיילות מלרנקה.')
      WHERE id = '64981eb1-e9a3-41a4-bd93-1fd4c78581d7';
    $sql$;
  END IF;
END $$;
