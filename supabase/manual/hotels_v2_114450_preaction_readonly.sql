...
-- Final successor 114450 preaction; checkpoint f8fe3765a431246adae09c8fc30418ba5cd86371.
...
-- Expected rows: 452 (451 required leaves and one summary).
...
WITH expected_functions(signature,catalog_sha) AS (VALUES
 ...
 UNION ALL SELECT 132+row_number() OVER(ORDER BY signature)::integer,'source_security',signature,'true',exact::text,exact IS TRUE FROM function_results
 UNION ALL SELECT 451,'source_security','complete_function_universe_exact','true',exact::text,exact IS TRUE FROM catalog_guard
), totals AS (
...
UNION ALL SELECT 'final_gate',452,'PREACTION_READY','true',ready::text,ready,ready,blockers,required_count,passed_count FROM totals ORDER BY ordinal;
ROLLBACK;
