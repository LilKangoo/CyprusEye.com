-- Hotels V2 ADMIN-A Room Type gallery repair preflight (READ ONLY).
-- Supabase SQL Editor compatible: no psql meta-commands.
-- Run immediately before migration 20260811330000.  Defaults are the exact
-- 2026-08-15 production baseline; disposable gates may override them with
-- session settings before including this file.
...
select set_config('hotels_v2.admin_a_expected_upper_rate_version',
  coalesce(nullif(current_setting('hotels_v2.admin_a_expected_upper_rate_version',true),''),'1'),false);
select set_config('hotels_v2.admin_a_expected_ground_rate_version',
  coalesce(nullif(current_setting('hotels_v2.admin_a_expected_ground_rate_version',true),''),'1'),false);
...
begin
  if to_regclass('public.hotel_partner_hotel_permissions') is null
    ...
    raise exception 'HOTELS_V2_ADMIN_A_PREFLIGHT_FAIL: H3.2A/H3.1P prerequisite missing';
  end if;
  ...
  if v_definition not like '%hotels_v2_h2b2_preserve_reviewed_property_policy_v1%'
    ...
    raise exception 'HOTELS_V2_ADMIN_A_PREFLIGHT_FAIL: exact repair boundary mismatch';
  end if;
  ...
  if not (select prosecdef from pg_catalog.pg_proc
    ...
    raise exception 'HOTELS_V2_ADMIN_A_PREFLIGHT_FAIL: RPC security contract mismatch';
  end if;
  ...
  if (select count(*) from public.site_settings)<>1
    ...
    raise exception 'HOTELS_V2_ADMIN_A_PREFLIGHT_FAIL: public flags/RLS contract mismatch';
  end if;
  ...
  if not exists(select 1 from public.hotels hotel where hotel.id=c_hotel
     ...
     or not exists(select 1 from public.hotel_room_types where id=c_upper
       and hotel_id=c_hotel and version=current_setting(
         'hotels_v2.admin_a_expected_upper_version')::bigint
     ...
     or not exists(select 1 from public.hotel_room_types where id=c_ground
       and hotel_id=c_hotel and version=current_setting(
         'hotels_v2.admin_a_expected_ground_version')::bigint
       and md5(gallery::text)=current_setting(
         'hotels_v2.admin_a_expected_ground_gallery_fingerprint')
       and status='active' and max_occupancy=4
       and inventory_mode='pooled' and base_inventory_count=1)
     or (select count(*) from public.hotel_room_types where hotel_id=c_hotel)<>2 then
    raise exception 'HOTELS_V2_ADMIN_A_PREFLIGHT_FAIL: exact property/Room Type baseline drift';
  end if;

  if not exists(select 1 from public.hotel_pricing_schedules where id=c_schedule
       and hotel_id=c_hotel
       ...
       and not is_active and review_status='reviewed' and source='legacy_preview'
     ...
     or not exists(select 1 from public.hotel_pricing_schedules where id=c_party
       ...
       and code='legacy-property-party-preview'
       and application_scope='property_booking_party' and maximum_party_size=8
     ...
     or not exists(select 1 from public.hotel_room_rates where id=c_upper_rate
       ...
       and pricing_schedule_id=c_schedule and not is_active
       and version=current_setting('hotels_v2.admin_a_expected_upper_rate_version')::bigint)
     or not exists(select 1 from public.hotel_room_rates where id=c_ground_rate
       ...
       and pricing_schedule_id=c_schedule and not is_active
       and version=current_setting('hotels_v2.admin_a_expected_ground_rate_version')::bigint)
  ...
  if not coalesce((v_snapshot->>'supported')::boolean,false)
     ...
     or v_snapshot#>>'{target,room_schedule,tier_fingerprint}'<>
       '49df574ed79950df08c2dd9d9ecb278c'
     ...
     or not exists(select 1 from public.hotel_pricing_promotion_reviews review
       where review.hotel_id=c_hotel
         ...
         and review.source_tier_fingerprint='eaba639f607f2e2b8acb61f512af2830'
         and review.target_fingerprint=current_setting(
           'hotels_v2.admin_a_expected_target_fingerprint')
         and review.pricing_occupancy_mapping_fingerprint=
           '6f6e6c64f0b0d0aa60e3575d4fd4ac1c'
         and review.parity_fingerprint='b3c915266ab060efaba522cf5587fb75'
    ...
    raise exception 'HOTELS_V2_ADMIN_A_PREFLIGHT_FAIL: reviewed H3.1P receipt/parity drift: %',
      v_snapshot->'blockers';
  end if;
  ...
  if (select count(*) from public.hotel_bookings)<>
    ...
    raise exception 'HOTELS_V2_ADMIN_A_PREFLIGHT_FAIL: protected history fingerprint drift';
  end if;
end
$admin_a_preflight$;

-- Capture this deterministic composite and compare it byte-for-byte with the
-- identically named outputs from the foundation and post-Admin verifiers.
-- The migration independently enforces the same no-change contract inside its
-- transaction; this output makes the cross-run operator comparison explicit.
...
select
  ...
  true as hotels_v2_admin_a_room_gallery_post_promotion_preflight_safe
