-- Special Offers 3C.5C-6I draft only.
-- Enforces one authenticated user entry per Special Offer campaign.
-- Prepared for manual review. Do not run automatically.
--
-- Scope:
-- - public.submit_special_offer_entry(text, text, jsonb, uuid)
-- - add a partial unique index on (offer_id, user_id) where user_id is not null
-- - preserve auth.uid(), confirmed email, canonical email and profile enrichment
-- - return an existing own entry as a safe idempotent/already-submitted response
--
-- Out of scope:
...
-- - draws
-- - winners
-- - changing or backfilling existing entries/answers/activities
...
begin
  if exists (
    ...
    raise exception 'single_entry_duplicates_exist' using errcode = '23505';
  end if;
end;
$$;

create unique index if not exists idx_special_offer_entries_offer_user_unique
  on public.special_offer_entries(offer_id, user_id)
...
exception
  when unique_violation then
    if v_offer.id is not null and v_uid is not null then
      select *
        into v_existing
      from public.special_offer_entries e
      where e.offer_id = v_offer.id
...
comment on function public.submit_special_offer_entry(text, text, jsonb, uuid) is
  'Atomically validates and stores an authenticated, confirmed-email Special Offers form entry. Returns an existing own entry for repeat submits and enforces one user entry per campaign.';
