...
--
-- This repair changes one RPC body only.  It accepts either the currently
-- deployed H2B.1 body or an already repaired H2B.2 body, and it fails closed
-- unless the accepted 7 Arches shadow graph remains public-inert.
with
constants as (
  select
...
rpc as (
  select
    ...
    count(*) filter(where
      ...
      and pg_get_functiondef(procedure_info.oid) like '%hotels_v2_h2b1_three_way_identity_v1%'
      ...
      and (
        ...
        or (
          pg_get_functiondef(procedure_info.oid) like '%hotels_v2_h2b1_seven_arches_child_policy_mismatch%'
          and pg_get_functiondef(procedure_info.oid) like '%update public.hotels set children_policy=''minimum_age'',minimum_child_age=10%'
        )
      )
    )::integer accepted_body_count,
    ...
    max(pg_get_functiondef(procedure_info.oid)) definition
...
room_state as (
  select
    ...
    ))::integer foreign_gallery_room_count
  ...
  select
    ...
    (select count(*) from public.hotel_room_rates room_rate
      where room_rate.id in (constants.upper_rate_id,constants.ground_rate_id)
        and room_rate.hotel_id=constants.hotel_id
        and room_rate.rate_plan_id=constants.plan_id
    ...
    (select count(*) from public.hotel_pricing_schedules schedule
      where schedule.id in (constants.room_schedule_id,constants.party_schedule_id)
        and schedule.hotel_id=constants.hotel_id
        and not schedule.is_active and schedule.review_status='requires_review')::integer inert_schedule_count,
...
evaluated as (
  select *,
    case when room_state.upper_policy_override is null
      then property_state.children_policy else room_state.upper_policy_override end upper_effective_policy,
    case when room_state.upper_policy_override is null
      then property_state.minimum_child_age else room_state.upper_minimum_age_override end upper_effective_minimum_age,
    case when room_state.ground_policy_override is null
      then property_state.children_policy else room_state.ground_policy_override end ground_effective_policy,
    case when room_state.ground_policy_override is null
      then property_state.minimum_child_age else room_state.ground_minimum_age_override end ground_effective_minimum_age
...
select
  ...
  children_policy property_children_policy,
  minimum_child_age property_minimum_child_age,
  upper_policy_override,upper_minimum_age_override,
  upper_effective_policy,upper_effective_minimum_age,
  upper_version,upper_updated_at,upper_status,upper_gallery_count,upper_amenities,
  ground_policy_override,ground_minimum_age_override,
  ground_effective_policy,ground_effective_minimum_age,
  ...
  (
    ...
    and safe_search_path_count=1 and accepted_body_count=1 and exact_grant_count=1
    ...
    and room_tier_count=27 and party_tier_count=63
    and all_off_count=1
  ...
  ) hotels_v2_h2b2_policy_preservation_preflight_safe
from evaluated;
