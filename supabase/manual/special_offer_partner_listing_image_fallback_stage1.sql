-- =====================================================
-- Special Offers 3C.8E - Partner listing image fallback patch
-- =====================================================
-- Prepared only. Do not run until preflight is reviewed.
--
-- Keeps the existing RPC signature and return type. The existing
-- cover_image_url payload field becomes the safe card image candidate:
-- cover_image_url -> hero_image_url -> meta_image_url.
-- =====================================================
...
begin
  ...
  if not exists (
    ...
    raise exception 'Missing required column public.special_offers.meta_image_url';
  end if;
end $$;
...
begin
  ...
  localized as (
    select
      ...
      nullif(btrim(coalesce(eo.cover_image_url, eo.hero_image_url, eo.meta_image_url, '')), '') as cover_image_url,
      eo.start_at,
      eo.end_at
...
comment on function public.get_partner_active_special_offers(text) is
  'Read-only active Special Offers listing for authenticated active partners. cover_image_url is the safe card image candidate: cover, hero, then meta image. Referral URLs are built in Partner Portal.';
