(function attachHotelsV2PartnerWorkspace(root, factory) {
  ...
  function renderPayments() {
    ...
    const connectLink = `<article class="partner-hotel-workspace__card partner-stripe-card" data-phw-stripe-lifecycle data-stripe-state="${html(stripeView.state)}"><h3>Stripe Connect</h3><p>${html(stripeView.label)}</p>${connection ? `<dl><div><dt>Partner onboarding authorization</dt><dd>${connection.onboarding_authorized ? 'Authorized' : 'Not authorized'}</dd></div><div><dt>Global Stripe capability</dt><dd>${connection.platform_enabled ? 'ON' : 'OFF'}</dd></div><div><dt>Account state</dt><dd>${html(connection.account_status)}</dd></div><div><dt>Platform readiness</dt><dd>${html(stripeView.readiness || 'UNKNOWN')}</dd></div></dl>` : ''}<p>One Partner-owned Standard account serves all assigned Hotels. Connection does not change payment routing or the EUR10 commission per allocated Room/night.</p>${stripeView.canConnect
      ? `<a class="partner-stripe-tile" data-phw-stripe-connection href="/partners/stripe-connect.html?partner=${encodeURIComponent(state.partnerId)}&amp;hotel=${encodeURIComponent(state.workspace.hotel_id)}&amp;lang=${encodeURIComponent(state.language)}"><img class="partner-stripe-logo" src="/assets/stripe-wordmark.svg" width="78" height="33" alt="Stripe"><span class="partner-stripe-tile__copy"><strong>${connection.account_status === 'ONBOARDING_INCOMPLETE' ? 'Continue setup' : 'Connect Stripe'}</strong><small>${html(text('stripeOnboarding'))}</small></span><span class="partner-stripe-tile__arrow" aria-hidden="true">→</span></a>` : ''}</article>`;
