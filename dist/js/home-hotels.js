...
function renderHomeHotels(){
  ...
  const renderCards = (visibleHotels) => visibleHotels.map((h, index)=>{
    ...
    return `
      <article
         class="hotel-home-card"
         data-home-hotel-index="${index}"
         role="button"
         tabindex="0"
         aria-label="${title}"
         style="position:relative;height:200px;border-radius:12px;overflow:hidden;cursor:pointer;transition:transform .3s,box-shadow .3s;box-shadow:0 4px 6px rgba(0,0,0,.1);text-decoration:none;display:block;border:none;background:transparent;padding:0;"
...
function bindHomeHotelCardTriggers(root) {
  ...
  scope.querySelectorAll('.hotel-home-card[data-home-hotel-index]').forEach((card) => {
    if (card.dataset.boundHotelCard === '1') return;
    card.dataset.boundHotelCard = '1';

    const openCard = () => {
      const index = Number(card.getAttribute('data-home-hotel-index'));
      if (!Number.isFinite(index)) return;
      const hotel = homeHotelsDisplay[index];
      if (!hotel) return;
      openHotelModalRecordInternal(hotel, { list: homeHotelsDisplay, index });
    };

    card.addEventListener('click', (event) => {
      const target = event.target;
      if (target instanceof Element && target.closest('[data-ce-save="1"]')) {
        return;
      }
      openCard();
    });
...
function initHomeHotels() {
  ...
  if (prevBtn) prevBtn.addEventListener('click', ()=>{
    ...
    if (nextHotel) openHotelModalRecordInternal(nextHotel, { list: homeHotelModalList, index: i });
  });
...
function getHomeHotelBookingUiApi() {
  return window.CE_HOTEL_BOOKING_UI || null;
}
...
function getHomeHotelQuote() {
  const homeHotelBookingUi = getHomeHotelBookingUiApi();
  const form = document.getElementById('hotelBookingForm');
  if (!homeCurrentHotel || !form || !homeHotelBookingUi?.calculateQuoteFromForm) {
    return {
      roomPricing: { total: 0, billableNights: 1, tier: null },
      roomTotal: 0,
      extrasTotal: 0,
      total: 0,
      baseTotal: 0,
  ...
  return homeHotelBookingUi.calculateQuoteFromForm(homeCurrentHotel, form, {
    nights,
    inputName: 'hotel_extra_ids',
  });
}

function renderHomeHotelBookingUiSections(options = {}) {
  const homeHotelBookingUi = getHomeHotelBookingUiApi();
  if (!homeCurrentHotel || !homeHotelBookingUi) return;
  const form = document.getElementById('hotelBookingForm');
...
function updateHotelLivePrice(){
  ...
  const quote = getHomeHotelQuote();
  const result = quote.roomPricing || calculateHotelPrice(homeCurrentHotel, persons, nights);
  const coupon = getHomeHotelCouponContext(quote.baseTotal || quote.total || 0);
  ...
  if (quote.extrasTotal > 0) {
    notes.push(hotelsTWithParams(
      'hotels.price.extrasIncluded',
      `Opłaty i dodatki: ${formatHomeHotelPrice(quote.extrasTotal)}. Cena pokoju: ${formatHomeHotelPrice(quote.roomTotal)}.`,
      { extras: formatHomeHotelPrice(quote.extrasTotal), room: formatHomeHotelPrice(quote.roomTotal) }
    ));
  }
...
function openHotelModalRecordInternal(hotelRecord, options = {}) {
  ...
  thumbsWrap.innerHTML = photos.map((u,i)=>`<img src="${getMediaViewerUrl(u)}" alt="miniatura" class="${i===0?'active':''}" data-src="${u}" />`).join('');
  thumbsWrap.querySelectorAll('img').forEach(el=>{
    el.addEventListener('click', ()=>{
      thumbsWrap.querySelectorAll('img').forEach(t=>t.classList.remove('active'));
      el.classList.add('active');
      imgEl.src = getMediaViewerUrl(el.dataset.src);
    });
...
window.openHotelModalHome = function(index){
  ...
  if (!h) return;
  openHotelModalRecordInternal(h, { list: homeHotelsDisplay, index });
}

window.openHotelModalByRecord = function(hotelRecord, options = {}) {
  openHotelModalRecordInternal(hotelRecord, options);
}
...
window.openHotelModalById = function(hotelId, options = {}) {
  ...
  const list = Array.isArray(options.list) && options.list.length
    ? options.list.filter(Boolean)
    : (Array.isArray(homeHotelsData) && homeHotelsData.length
      ? homeHotelsData.slice()
      : (Array.isArray(mapHotels) && mapHotels.length ? mapHotels.slice() : []));
  const hotelRecord = list.find((entry) => String(entry?.id || '').trim() === id)
    || homeHotelsData.find((entry) => String(entry?.id || '').trim() === id)
    || mapHotels.find((entry) => String(entry?.id || '').trim() === id)
    || null;
  if (!hotelRecord) return;
  const index = Math.max(0, list.findIndex((entry) => String(entry?.id || '').trim() === id));
  openHotelModalRecordInternal(hotelRecord, { list: list.length ? list : [hotelRecord], index });
}
...
// Lightbox (gallery)
let lbIndex = 0;
let homeHotelLightboxPanoramaCleanup = null;
function getHotelPhotos(){ return Array.isArray(homeCurrentHotel?.photos)? homeCurrentHotel.photos: []; }
function getMediaViewerUrl(url) {
  const viewer = window.CE_MEDIA_VIEWER;
  if (viewer?.getDisplayUrl) return viewer.getDisplayUrl(url);
  return String(url || '').split('#')[0];
}

function normalizeMediaCompareUrl(url) {
  const viewer = window.CE_MEDIA_VIEWER;
  if (viewer?.normalizeForCompare) return viewer.normalizeForCompare(url);
  return String(url || '').split('#')[0];
}

function clearHomeHotelLightboxPanorama() {
  const pano = document.getElementById('lbPano');
  if (!pano) return;
  if (typeof homeHotelLightboxPanoramaCleanup === 'function') {
    try {
      homeHotelLightboxPanoramaCleanup();
    } catch (_) {
  ...
  homeHotelLightboxPanoramaCleanup = null;
  pano.hidden = true;
  pano.dataset.cePanoramaActive = '0';
}

function currentHotelPhotoIndex() {
  const imageEl = document.getElementById('modalHotelImage');
  const currentSrc = normalizeMediaCompareUrl(imageEl?.src || '');
  const arr = getHotelPhotos();
  const i = arr.findIndex((url) => normalizeMediaCompareUrl(url) === currentSrc);
  return i >= 0 ? i : 0;
}

async function showHotelLightbox(i) {
  const arr = getHotelPhotos();
  if (!arr.length) return;
  lbIndex = (i + arr.length) % arr.length;
  const mediaUrl = arr[lbIndex];
  const img = document.getElementById('lbImg');
  const pano = document.getElementById('lbPano');
  const viewer = window.CE_MEDIA_VIEWER;
  const isPanorama = Boolean(viewer?.isPanorama?.(mediaUrl));

  clearHomeHotelLightboxPanorama();

  if (img) {
    img.hidden = isPanorama;
    img.src = getMediaViewerUrl(mediaUrl);
  }

  if (isPanorama && pano && viewer?.mountPanorama) {
    pano.hidden = false;
    try {
      const mounted = await viewer.mountPanorama(pano, mediaUrl);
      if (mounted?.isPanorama) {
        homeHotelLightboxPanoramaCleanup = mounted.destroy;
      }
    } catch (error) {
      console.warn('Hotel panorama lightbox mount failed:', error);
      pano.hidden = true;
      pano.dataset.cePanoramaActive = '0';
...
function closeHotelLightbox(){ const lb=document.getElementById('imgLightbox'); if(!lb) return; clearHomeHotelLightboxPanorama(); const img=document.getElementById('lbImg'); if(img){ img.hidden=false; img.src=''; } lb.classList.remove('active'); lb.hidden=true; document.removeEventListener('keydown', onHotelLbKey); }
